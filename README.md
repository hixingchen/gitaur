...
当同步检测（rebase/merge）产生冲突时，应用提供完整的冲突解决流程：

1. 冲突面板自动弹出，显示冲突文件列表和解决进度
2. 点击冲突文件，在 DiffView 中查看并编辑冲突内容
3. 删除冲突标记（`<<<<<<<` `=======` `>>>>>>>`），保留正确内容
4. 保存文件（自动暂存）
5. 全部解决后，点击 **"已解决，继续"** 完成变基/合并
6. 如果设置了自动合并，已创建的 MR 会自动合并

**注意**：变基冲突期间无法切换任务，需先解决或取消变基。
